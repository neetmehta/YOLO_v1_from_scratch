...
def convert_cellboxes(predictions, S=7):
    ...
    predictions = predictions.reshape(batch_size, 6, 20, 19)
    bboxes1 = predictions[..., 10:14]
    bboxes2 = predictions[..., 15:19]
    scores = torch.cat(
        (predictions[..., 9].unsqueeze(0), predictions[..., 14].unsqueeze(0)), dim=0
    )
    best_box = scores.argmax(0).unsqueeze(-1)
    best_boxes = bboxes1 * (1 - best_box) + best_box * bboxes2
    cell_indices = torch.arange(20).repeat(batch_size, 6, 1).unsqueeze(-1)
    x = 1 / 20 * (best_boxes[..., :1] + cell_indices)
    y = 1 / 6 * (best_boxes[..., 1:2] + cell_indices.permute(0, 2, 1, 3))
    w_y = 1 / S * best_boxes[..., 2:4]
    converted_bboxes = torch.cat((x, y, w_y), dim=-1)
    predicted_class = predictions[..., :9].argmax(-1).unsqueeze(-1)
    best_confidence = torch.max(predictions[..., 9], predictions[..., 14]).unsqueeze(
        -1
    )
...
def cellboxes_to_boxes(out, S=7):
    converted_pred = convert_cellboxes(out).reshape(out.shape[0], 6*20, -1)
